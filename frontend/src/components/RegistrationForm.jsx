...
const RegistrationForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        setSubmitMessage('Registration successful! Check your email for verification.');
        onSuccess(); // Call the success callback to redirect to verification page
      } else {
        const error = await response.json();
        setSubmitMessage(error.detail || error.message || 'Registration failed');
      }
    } catch (error) {
